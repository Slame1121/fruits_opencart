...
class ModelDesignLayout extends Model {
    ...
    public function getRoute($layout_id) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "layout_route WHERE layout_id = " . $this->db->escape($layout_id) . " AND store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY route DESC LIMIT 1");
