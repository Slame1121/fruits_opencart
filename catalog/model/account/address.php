...
class ModelAccountAddress extends Model {
	public function addAddress($customer_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . ( isset($data['firstname']) ? $this->db->escape($data['firstname']) : '') . "', lastname = '" .( isset($data['lastname']) ? $this->db->escape($data['lastname']) : '') . "', company = '" .( isset($data['company']) ? $this->db->escape($data['company']) : '') . "', address_1 = '" . ( isset($data['address_1']) ? $this->db->escape($data['address_1']) : ''). "', address_2 = '" . ( isset($data['address_2']) ? $this->db->escape($data['address_2']) : ''). "', postcode = '" .( isset($data['postcode']) ? $this->db->escape($data['postcode']) : '')  . "', city = '" . ( isset($data['city']) ? $this->db->escape($data['city']) : '') . "', zone_id = '" .  ( isset($data['zone_id']) ? (int)$data['zone_id'] : 0)  . "', country_id = '" .( isset($data['country_id']) ?  (int)$data['country_id'] : 0) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['address']) ? json_encode($data['custom_field']['address']) : '') . "'");
