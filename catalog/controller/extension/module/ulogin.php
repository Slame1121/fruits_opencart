...
class ControllerExtensionModuleUlogin extends Controller {
	...
	public function index($setting) {
		...
		if (empty($ulogin_type) && !$this->userIsLogged && !empty($ulogin_status)) {
			...
			$tpl .= 'module/ulogin/ulogin_form';


		} elseif (!$this->userIsLogged && $ulogin_type == 'offline') {
			$tpl .= 'module/ulogin/ulogin_panel';
			...
			$tpl .= 'module/ulogin/ulogin_profile';
	...
	public function messager() {
        ...
        $html .= $this->load->view('module/ulogin/ulogin_message', $data_message);
...
//=================================================================================
	...
	protected function sendMessage ($params = array()) {
		...
		if ($this->doRedirect) {
			...
			$this->response->redirect($this->url->link($redirect, '', true));
	...
	protected function loginUser($user_id = 0) {
		...
		if (empty($customer_info['email']) && !empty($u_data['email']))	{
			$customer_info['email'] = $u_data['email'];
		}
		if (empty($customer_info['firstname']) && !empty($u_data['first_name']))	{
			$customer_info['firstname'] = $u_data['first_name'];
		}
        ...
        //var_dump($u_data,$customer_info);die;
		// вход в систему
